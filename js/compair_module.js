...
module.exports = function compair(xlsx, xml) {
    ...
    let delStr = "";
    let warStr = "";
    if (idChanged.length == 0 && nameChanged.length == 0 && added.length == 0 && deleted.length == 0 && warning.length == 0) {
        count++;
    }
    else {
        //id变化数据
        if (idChanged.length == 0) {
            ...
            for (let i = 0; i < idChanged.length; i++) {
...
function outputRes(str) {
    fs.appendFile("./txt/Result.txt", str, function (err) {
        if (err) {
            console.log(err);
        } else {
